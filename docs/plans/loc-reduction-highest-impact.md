...
| 1.3 | Collapse transcript bridge ref        | `use-agent-impl-core.tsx`                                                     | Optional: pass orchestration callbacks directly to `useBackendEventSubscription` without `transcriptBridgeRef` indirection (~30 lines)                                                 |
| 1.4 | `transcript.message` ingest           | `use-active-session-transcript-orchestration.ts`                              | Either apply `event.entry` via store upsert or delete no-op `return true` with comment + test that bus never sends `transcript.message` (see `server/projected-transcript-publish.ts`) |
