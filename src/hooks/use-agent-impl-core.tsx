...
/**
 * Returns true when configured backend server points to local machine.
 * Used to decide whether native Electron dialogs make sense (local) or whether
 * user should type remote path instead.
 */
...
export function InternalAgentProvider({
	...
	const sendPrompt = useCallback(
		...
		[runtime, dispatchPromptDirect, ensureSessionFromDraft, selectSession],
	);
	...
	const findFiles = useCallback(
		async (directory: string | null, query: string): Promise<string[]> => {
			...
			const workspaceId = stateRef.current.activeWorkspaceId;
			try {
				return await runtime.findFiles(
					{ directory: directory ?? undefined, workspaceId },
					query,
				);
			} catch (error) {
	...
	const summarizeSession = useCallback(async () => {
		...
		try {
			...
			const messages = (await runtime.getMessages(sessionId, { limit: 100 }))
				.messages;
			dispatch({
				type: "SET_MESSAGES",
	...
	const revertToMessage = useCallback(
		async (messageID: string) => {
			...
			try {
				const session = await runtime.revertSession(
					state.activeSessionId,
					messageID,
				);
