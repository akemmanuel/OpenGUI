...
type MarkdownPlugin = NonNullable<Parameters<typeof ReactMarkdown>[0]["remarkPlugins"]>[number];
type HtmlPlugin = NonNullable<Parameters<typeof ReactMarkdown>[0]["rehypePlugins"]>[number];
...
function ensureKatexStylesheet() {
  ...
  if (
    document.head.querySelector(`link[data-katex-stylesheet="true"][href="${katexStylesheetUrl}"]`)
  ) {
...
function cleanContent(raw: string): string {
  return raw
    ...
    .replace(/(```[\s\S]*?```|`[^`\n]+`)|( - )/g, (match, codeBlock: string | undefined) =>
      codeBlock ? match : " - ",
    );
}
...
export const MarkdownRenderer = memo(function MarkdownRenderer({ content }: { content: string }) {
  ...
  const remarkPlugins = mathPlugins ? [remarkGfm, mathPlugins.remarkMath] : [remarkGfm];
