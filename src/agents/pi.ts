...
const { compose: toCompositeSessionId, decompose: toRawSessionId } = createBackendIdCodec("pi");

function tagPiSession(
  session: TaggedSession,
  target?: { directory?: string; workspaceId?: string },
): TaggedSession {
  return tagBackendSession("pi", session, target);
}
  ...
  return {
    ...
    runtime: {
      ...
      getMessages: async (sessionId, options) => {
        ...
        return {
          ...
          messages: page.messages.map((entry) => ({
            info: normalizeMessageSessionId("pi", entry.info),
            parts: entry.parts.map((part) => normalizePartSessionId("pi", part)),
          })),
        };
      },
      prompt: async ({
      ...
      compactSession: async (sessionId, model, target) => {
        const { directory, workspaceId } = getTarget(target);
        requireSuccess(
          await bridge.summarizeSession(toRawSessionId(sessionId), model, directory, workspaceId),
