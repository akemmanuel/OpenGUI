...
export function pruneSessionDraftImages(images: SessionDraftImagesMap): SessionDraftImagesMap {
  const pruned: SessionDraftImagesMap = {};
  for (const [key, value] of Object.entries(images)) {
    const next = value.filter((image) => typeof image === "string" && image.trim().length > 0);
    if (next.length > 0) {
      pruned[key] = next;
    }
  }
