...
function baseAgent(prefix: "claude-code" | "pi" | "codex") {
  return {
    ...
    removeProject: (directory: string, workspaceId?: string) =>
      invoke(`${prefix}:project:remove`, directory, workspaceId),
    disconnect: () => invoke(`${prefix}:disconnect`),
    listSessions: (directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:session:list`, directory, workspaceId),
    createSession: (title?: string, directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:session:create`, title, directory, workspaceId),
    deleteSession: (sessionId: string, directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:session:delete`, sessionId, directory, workspaceId),
    updateSession: (sessionId: string, title: string, directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:session:update`, sessionId, title, directory, workspaceId),
    getSessionStatuses: (directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:session:statuses`, directory, workspaceId),
    forkSession: (
      sessionId: string,
      messageID?: string,
      directory?: string,
      workspaceId?: string,
    ) => invoke(`${prefix}:session:fork`, sessionId, messageID, directory, workspaceId),
    getProviders: (directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:providers`, directory, workspaceId),
    getAgents: (directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:agents`, directory, workspaceId),
    getCommands: (directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:commands`, directory, workspaceId),
    getMessages: (sessionId: string, options?: unknown, directory?: string, workspaceId?: string) =>
      invoke(`${prefix}:messages`, sessionId, options, directory, workspaceId),
    ...
    respondPermission: (sessionId: string, permissionId: string, response: unknown) =>
      invoke(`${prefix}:permission`, sessionId, permissionId, response),
    sendCommand: (
      sessionId: string,
      command: string,
      args: unknown,
    ...
    summarizeSession: (
      ...
      directory?: string,
      workspaceId?: string,
    ) => invoke(`${prefix}:session:summarize`, sessionId, model, directory, workspaceId),
    findFiles: (directory?: string, workspaceId?: string, query?: string) =>
      invoke(`${prefix}:find:files`, directory, workspaceId, query),
...
export function installWebElectronAPI() {
  ...
  const api = {
    ...
    opencode: {
      ...
      removeProject: (directory: string, workspaceId?: string) =>
        invoke("opencode:project:remove", directory, workspaceId),
      disconnect: () => invoke("opencode:disconnect"),
      listSessions: (directory?: string, workspaceId?: string) =>
        invoke("opencode:session:list", directory, workspaceId),
      createSession: (title?: string, directory?: string, workspaceId?: string) =>
        invoke("opencode:session:create", title, directory, workspaceId),
      ...
      getSessionStatuses: (directory?: string, workspaceId?: string) =>
        invoke("opencode:session:statuses", directory, workspaceId),
      revertSession: (id: string, messageID: string, partID?: string) =>
        invoke("opencode:session:revert", id, messageID, partID),
      unrevertSession: (id: string) => invoke("opencode:session:unrevert", id),
      forkSession: (id: string, messageID?: string) =>
        invoke("opencode:session:fork", id, messageID),
      getProviders: (directory?: string, workspaceId?: string) =>
        invoke("opencode:providers", directory, workspaceId),
      listAllProviders: (directory?: string, workspaceId?: string) =>
        invoke("opencode:provider:list", directory, workspaceId),
      getProviderAuthMethods: (directory?: string, workspaceId?: string) =>
        invoke("opencode:provider:auth-methods", directory, workspaceId),
      connectProvider: (
        directory: string | undefined,
        workspaceId: string | undefined,
        providerID: string,
        auth: unknown,
      ) => invoke("opencode:provider:connect", directory, workspaceId, providerID, auth),
      disconnectProvider: (
        directory: string | undefined,
        workspaceId: string | undefined,
        providerID: string,
      ) => invoke("opencode:provider:disconnect", directory, workspaceId, providerID),
      oauthAuthorize: (
        directory: string | undefined,
        workspaceId: string | undefined,
        providerID: string,
        method: unknown,
      ) => invoke("opencode:provider:oauth:authorize", directory, workspaceId, providerID, method),
      oauthCallback: (
        directory: string | undefined,
        workspaceId: string | undefined,
        providerID: string,
      ...
      disposeInstance: (directory?: string, workspaceId?: string) =>
        invoke("opencode:instance:dispose", directory, workspaceId),
      getAgents: (directory?: string, workspaceId?: string) =>
        invoke("opencode:agents", directory, workspaceId),
      getMessages: (
        sessionId: string,
        options?: unknown,
        directory?: string,
        workspaceId?: string,
      ) => invoke("opencode:messages", sessionId, options, directory, workspaceId),
      prompt: (
        sessionId: string,
        text: string,
        images: unknown,
        ...
        agent: unknown,
        variant: unknown,
      ) => invoke("opencode:prompt", sessionId, text, images, model, agent, variant),
      abort: (sessionId: string) => invoke("opencode:abort", sessionId),
      respondPermission: (sessionId: string, permissionId: string, response: unknown) =>
        invoke("opencode:permission", sessionId, permissionId, response),
      getCommands: (directory?: string, workspaceId?: string) =>
        invoke("opencode:commands", directory, workspaceId),
      sendCommand: (
        sessionId: string,
        command: string,
        args: unknown,
        ...
        agent: unknown,
        variant: unknown,
      ) => invoke("opencode:command:send", sessionId, command, args, model, agent, variant),
      summarizeSession: (sessionId: string, model: unknown) =>
        invoke("opencode:session:summarize", sessionId, model),
      replyQuestion: (requestID: string, answers: unknown) =>
        invoke("opencode:question:reply", requestID, answers),
      rejectQuestion: (requestID: string) => invoke("opencode:question:reject", requestID),
      getMcpStatus: (directory?: string, workspaceId?: string) =>
        invoke("opencode:mcp:status", directory, workspaceId),
      addMcp: (
        directory: string | undefined,
        workspaceId: string | undefined,
        name: string,
        config: unknown,
      ) => invoke("opencode:mcp:add", directory, workspaceId, name, config),
      connectMcp: (directory: string | undefined, workspaceId: string | undefined, name: string) =>
        invoke("opencode:mcp:connect", directory, workspaceId, name),
      disconnectMcp: (
        directory: string | undefined,
        workspaceId: string | undefined,
        name: string,
      ) => invoke("opencode:mcp:disconnect", directory, workspaceId, name),
      getConfig: (directory?: string, workspaceId?: string) =>
        invoke("opencode:config:get", directory, workspaceId),
      updateConfig: (
        directory: string | undefined,
        workspaceId: string | undefined,
        config: unknown,
      ) => invoke("opencode:config:update", directory, workspaceId, config),
      findFiles: (directory?: string, workspaceId?: string, query?: string) =>
        invoke("opencode:find:files", directory, workspaceId, query),
      getSkills: (directory?: string, workspaceId?: string) =>
        invoke("opencode:skills", directory, workspaceId),
