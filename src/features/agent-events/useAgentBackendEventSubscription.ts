...
} from "@/hooks/projected-transcript-events";
...
export function useBackendEventSubscription(input: {
  ...
  const handleBackendEvent = useCallback(
    (event: BackendEventEnvelope) => {
      ...
      const projected = tryParseProjectedTranscriptEvent(event);
      if (projected) {
        if (
          isExpectedProjectedTranscriptScope(projected.scope, tracking.expectedProjectKeys.current)
        ) {
          transcriptHandlers.ingestProjectedTranscriptEvent(projected);
        }
        return;
      }
      handleHarnessEvent({
